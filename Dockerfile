...
RUN if ! grep -q "STATIC_ROOT" geoface_attendance/settings.py; then \
        echo "STATIC_ROOT = BASE_DIR / 'staticfiles'" >> geoface_attendance/settings.py; \
    fi
...
EXPOSE 8000
CMD ["daphne", "-b", "0.0.0.0", "-p", "8000", "geoface_attendance.asgi:application"]